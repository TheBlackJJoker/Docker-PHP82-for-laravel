...
laravel: check-env check-laravel up
	...
	docker exec --user "${CURRENT_USER_ID}:${CURRENT_USER_GROUP_ID}" ${DOCKER_PHP_FPM} bash -c "mv -v ./Laravel/.[!.]* ./"
	docker exec --user "${CURRENT_USER_ID}:${CURRENT_USER_GROUP_ID}" ${DOCKER_PHP_FPM} bash -c "chmod -R 775 storage bootstrap/cache"
	docker exec --user "${CURRENT_USER_ID}:${CURRENT_USER_GROUP_ID}" ${DOCKER_PHP_FPM} bash -c "chown -R www-data:www-data storage bootstrap/cache"
